...
export const main = {
    ...
    tools: {
        getCurrentDatetime: {
            method: 'GET',
            path: '/',
            description: 'Get current date and time. Use this when you need to know the current date for travel queries, weather lookups, schedule searches, or ticket bookings. Supports multiple output formats and timezones. Use addToDatetime for related data. Use formatDatetime for related data.',
            parameters: [
                {
                    position: {
                        key: 'timezone',
                    ...
                    z: {
                        primitive: 'string()',
                        options: ['optional()', 'default(Europe/Berlin)']
                    }
                },
                {
                    position: {
                    ...
                    z: {
                        primitive: 'enum(iso,date,time,datetime,german,compact,unix,flixbus)',
                        options: ['optional()', 'default(iso)']
                    }
                }
            ],
            tests: [
                {
                    _description: 'Get current datetime in ISO format',
                    format: 'iso'
                },
                {
                    _description: 'Get current date only',
                    format: 'date'
                },
                {
                    _description: 'Get current time in German format',
                    format: 'german',
                    timezone: 'Europe/Berlin'
                },
                {
                    _description: 'Get FlixBus date format',
                    format: 'flixbus'
                },
                {
                    _description: 'Get Unix timestamp',
                    format: 'unix'
                }
            ],
            output: {
                ...
                schema: {
                    type: 'object',
                    description: 'Response from getCurrentDatetime',
                    properties: {
                        result: {
                            type: 'object',
        ...
        addToDatetime: {
            method: 'GET',
            path: '/add',
            description: 'Add hours, days, or minutes to a date. Use this for relative time expressions like "tomorrow", "in 2 hours", "next week". If no base date is given, uses current time. Returns result in requested format. Use getCurrentDatetime for related data. Use formatDatetime for related data.',
            parameters: [
                {
                    position: {
                        key: 'date',
            ...
            tests: [
                {
                    _description: 'Add 1 day (tomorrow)',
                    days: 1,
                    format: 'date'
                },
                {
                    _description: 'Add 2 hours to now',
                    hours: 2,
                    format: 'datetime'
                },
                {
                    _description: 'Add 3 days in German format',
                    days: 3,
                    format: 'german'
                },
                {
                    _description: 'Add 30 minutes',
                    minutes: 30,
                    format: 'time'
                },
                {
                    _description: 'Add 1 day in FlixBus format',
                    days: 1,
                    format: 'flixbus'
                }
            ],
            output: {
                ...
                schema: {
                    type: 'object',
                    description: 'Response from addToDatetime',
                    properties: {
                        result: {
                            type: 'object',
                            description: 'Result data from the API'
        ...
        formatDatetime: {
            method: 'GET',
            path: '/format',
            description: 'Convert a date string between formats. Use this when you have a date in one format and need it in another format for a different API. Supports ISO, German (DD.MM.YYYY), compact (YYYYMMDD), FlixBus (DD.MM.YYYY), and more. Use getCurrentDatetime for related data. Use addToDatetime for related data.',
            parameters: [
                {
                    position: {
                        key: 'date',
            ...
            tests: [
                {
                    _description: 'ISO to German format',
                    date: '2026-03-18',
                    outputFormat: 'german'
                },
                {
                    _description: 'ISO to compact',
                    date: '2026-03-18T14:30:00',
                    outputFormat: 'compact'
                },
                {
                    _description: 'German to ISO',
                    date: '18.03.2026',
                    outputFormat: 'iso'
                },
                {
                    _description: 'ISO to FlixBus format',
                    date: '2026-03-20',
                    outputFormat: 'flixbus'
                },
                {
                    _description: 'ISO to datetime',
                    date: '2026-03-18T22:30:00+01:00',
            ...
            output: {
                ...
                schema: {
                    type: 'object',
                    description: 'Response from formatDatetime',
                    properties: {
                        result: {
                            type: 'object',
                            description: 'Result data from the API'
